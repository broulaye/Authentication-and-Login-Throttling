<?php
  ...
  function validate_user($user, $errors=array()) {
    ...
    if(!is_blank($user['password'])) {
      ...
      elseif(!preg_match("#[a-z]+#", $user['password'])) {
          ...
          $errors[] = "Your Password must contain at least one symbol.";
      }

      if(is_blank($user['password_confirmation'])) {
        $errors[] = "Password confirmation cannot be blank.";
      } elseif ($user['password_confirmation'] != $user['password']) {
  ...
  function update_user($user) {
    ...
    if($user['password']) {
      ...
      //echo $prev_user['hashed_password'];
      if(!password_verify($prev_user['hashed_password'], $user['prev_password'])) {
        $errors[] = "Previous password doesn't match.";
        return $errors;
      }
    }
    $password = hash_password($user['password']);
    ...
    $sql .= "hashed_password = COALESCE('" . db_escape($db, $password) . "', users.hashed_password) ";
  ...
  // Add a new failed login to the table
